jQuery(document).ready(function ($) {
    ...
    if ($('form#contact_form').length > 0) {
        $('form#contact_form').validate({
            messages: {}, submitHandler: function (form) {
                $.ajax({
                    type: 'POST', url: 'send.php', data: $(form).serialize(), success: function (data) {
    ...
    if ($('#slides').length > 0) {
        ...
        $('#slides').superslides({animation: 'fade', hashchange: false, play: 5000});
    ...
    if ($('.slides').length > 0) {
        $('.slides').slick({autoplay: true, pauseOnHover: false, dots: true, speed: 1000, arrows: false});
    }
    new WOW().init();
    var bars = $(".bars");
    for (var i = 0; i < bars.length; i++) {
        ...
        for (var j = 0; j < highlights.length; j++) {
            ...
            $(highlight).appear(function () {
                ...
                $(this).animate({'width': percent + '%'}, 1700, function () {
    ...
    $(".milestone").appear(function () {
        $('.number', $(this)).countTo({speed: 1400});
    });
    if ($('.clients').length > 0) {
        $('.clients').slick({
            slidesToShow: 5,
